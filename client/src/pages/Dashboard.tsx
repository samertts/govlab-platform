...
export default function Dashboard() {
  ...
  const stats = [
    { label: "Pending Tests", value: samples?.filter((s: any) => s.status !== "completed").length || 0, icon: Clock, color: "text-amber-500", bg: "bg-amber-50" },
    { label: "Completed Today", value: samples?.filter((s: any) => s.status === "completed").length || 0, icon: FileCheck, color: "text-green-500", bg: "bg-green-50" },
    { label: "Urgent/STAT", value: samples?.filter((s: any) => s.priority !== "routine").length || 0, icon: AlertCircle, color: "text-red-500", bg: "bg-red-50" },
        ...
        <Card className="col-span-1 lg:col-span-2 border shadow-sm">
          ...
          <CardContent>
            <div className="space-y-4">
              ...
              ) : samples?.slice(0, 5).map((sample: any) => (
